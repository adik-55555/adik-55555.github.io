...
#  обьявляем класс формы описывающую формы добавления в статьи: lass AddPostForm(forms.Form): 
# эта форма не связанна с моделями поэтому потом была заменена на связанную с моделями class # #AddPostForm(forms.ModelForm)
# ниже атрибуты класса(поля) названия те же что и в классе #Uroki
...
# базой данных напрямую если он с ней взаимодействует поэтому для взаимодействия с базой данных #этот класс в дальнейшем
# унаследуем от другого класса - class AddPostForm(forms.ModelForm) см.внизу:
...
class AddPostForm(forms.Form):
    ...
    cat = forms.ModelChoiceField(queryset=Categor.objects.all(), label="Категории", empty_label="Категория не выбрана")
...
# форма связанная с моделями - наследуется от другого класса-ModelForm(когда нужно связать с моделью)
    def __init__(self, *arqs, **kwards): # конструктор базового класса-ModelForm
...
# стандартной валидации... обязательно clean затем имя поля...
